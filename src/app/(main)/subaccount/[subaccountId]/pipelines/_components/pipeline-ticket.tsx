...
const PipelineTicket = ({
  ...
  const handleDeleteTicket = async () => {
    try {
      setAllTickets((tickets: TicketWithTags) => tickets.filter(t => t.id !== ticket.id));
      const response = await deleteTicket(ticket.id);
      toast({
        title: 'Deleted Ticket',
        description: 'Deleted Ticket from the lane!',
      })
      ...
      {(provided, snapshot) => {
        ...
        return (
          ...
          >
            <AlertDialog>
              <DropdownMenu>
                <Card className='my-2 dark:bg-slate-900 bg-white shadow-none transition-all'>
